...
import { SOCKET_MANAGER_KEY, APP_DB_KEY } from '@/injection-keys.js'
...
export function createMockSocketManager(
  ...
  const socketManagerRef = ref(socketManager)
      ...
      [APP_DB_KEY as symbol]:
        ...
        ({
          localSetting: {
            ...
            where: () => ({
              equals: () => ({
                first: () => ({ isErr: () => false, value: undefined }),
              }),
            }),
          },
        } as const),
  ...
  return {
    socket: socketManager.settingsSocket,
    socketManager,
    socketManagerRef,
    global,
  }
