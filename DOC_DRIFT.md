...
- **Project Standards:** `README.md` script descriptions still referenced `XO` instead of the project's current `ESLint` standard.
- **Sync Design:** `docs/SYNC_DESIGN.md` noted `src/sync/sync-queue.ts` as a placeholder, but it had become a literal duplicate of the sync store; clarified its status as reserved for future use.
- **Debt Tracking:** `UNFINISHED.md` still listed "OAuth Callback handling" as unfinished, but implementation in `AuthCallbackView.vue` is complete.
- **Cleanup:** Removed stale drift reports for `ENVS.md`, `ARCHITECTURE.md`, and `AUTH.md` that were already resolved in previous passes.
