...
export class SyncManager {
  ...
  private async _applyPayload(payload: SyncPayload): Promise<Result<void, DbError | SyncError>> {
    // Tombstone handling
  ...
  private async _updateFromPayload(
    ...
    for (const msg of payload.messages) {
      ...
      if (r.isErr()) {
        this._logger.error('SyncManager: failed to re-insert message from remote', {
          error: r.error,
        })
      }
    }
