...
export class GitHubSyncAdapter implements ISyncAdapter {
  ...
  async pull(): Promise<Result<SyncPayload[], SyncError>> {
    ...
    try {
      ...
      return err(
        new SyncError(
          'GitHub pull: failed to parse directory listing',
    ...
    for (const file of files) {
      ...
      const fetchResult = await ResultAsync.fromPromise(
        ...
        (e) =>
          new SyncError(
            `GitHub pull: failed to fetch ${file.name}`,
            e instanceof Error ? e : undefined
          )
      )
  ...
  private async _getFile(
    ...
    try {
      ...
      return err(
        new SyncError('GitHub GET: failed to parse response', e instanceof Error ? e : undefined)
      )
    }
  }
