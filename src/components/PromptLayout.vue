<template>
  ...
  >
    ...
    >
      <div
        :class="[
          ...
          isDragging ? 'bg-accent ' + (direction === 'vertical' ? 'w-16' : 'h-16') : 'bg-borderMuted group-hover:bg-accent ' + (direction === 'vertical' ? 'group-hover:w-16' : 'group-hover:h-16')
        ]"
      />
    </div>
    ...
    <div
      ...
      :class="direction === 'vertical' ? 'border-t border-borderMuted' : 'pl-4 border-l border-borderMuted'"
      :style="rightStyle"
    >
      <slot name="detail" />
    </div>
