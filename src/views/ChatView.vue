<template>
  <section
    id="chat"
    class="flex flex-col w-full p-4 bg-slate"
  >
    ...
    <div
      v-if="error"
      class="bg-panel border border-error rounded-md p-3 mb-4"
    >
    ...
    <div class="flex flex-col gap-0 pt-4">
      ...
      <div class="flex flex-col sm:flex-row border border-borderMuted rounded bg-panel">
        <div class="flex flex-col sm:flex-row items-center justify-between gap-3 w-full">
          ...
          <div
            class="flex w-auto justify-center px-3 py-2 items-center hover:text-accent transition-colors"
            :class="{ 'opacity-50 cursor-not-allowed': isLoading || !currentMessage.trim() }"
            @click="handleSendMessage"
          >
            <i
              v-if="!isLoading"
              class="i-bi:play-circle text-xl text-accent"
            />
            <div
              v-else
              class="w-5 h-5"
