        ...
        <Form v-else :validation-schema="validationSchema">
          <div v-for="setting in filteredSettings" :key="setting.settingKey">
            ...
            <SettingField
              ...
              @field-changed="_handleFieldChange"
            />
          </div>
        </Form>
      </div>
...
const { settings, updateSetting, setApiKey, loadSettings } =
  useSettingsSocket();
...
const filteredSettings = computed<Setting[]>(
  () => settings.value as unknown as Setting[],
);
...
const _validateAndSaveApiKey = async (provider: string, apiKey: string) => {
  ...
  try {
    ...
    if (isValid) {
      ...
      validationErrors.value[provider] =
        `Invalid ${providerConfig.name} API key`;
    }
  } catch {
    validationStatus.value[provider] = "invalid";
    validationErrors.value[provider] =
      `Failed to validate ${providerConfig.name} API key`;
  }
};
...
const _handleFieldChange = async (fieldName: string, value: string) => {
  ...
  const setting = filteredSettings.value.find(
    (s: Setting) => s.settingKey === fieldName,
  );
  if (setting) {
    await updateSetting(
      fieldName,
